...
--2.Update an Existing Member's Address (Alice Johnson new address is 80 Main St):
UPDATE members
SET member_address = '80 Main St'
...
--8.Find Total Rental Income by History Category:
...
LEFT JOIN return_status as re_s
ON re_s.issued_id = ist.issued_id
WHERE re_s.return_id IS NULL;
...
/*14.Write a query to update the status of books in the books table to "Yes" when they are returned 
...
/*15.Create a query that generates a performance report for each branch, showing the number of books issued,
...
/*16.Use the CREATE TABLE AS (CTAS) statement to create a new table active_members containing members who have 
